...
from app.schemas.blogs import BlogCreate, BlogUpdate, BlogBase, BlogResponse
...
class CRUDBlog(CRUDBase[Blogs, BlogCreate, BlogUpdate]):
    ...
    def create(self, db: Session, *, obj_in: BlogCreate) -> Blogs:
        db_obj = self.model(
            member_id=obj_in.member_id,
            name=obj_in.name,
            blog_url=obj_in.blog_url,
            blog_icon=obj_in.blog_icon,
            blog_favicon=obj_in.blog_favicon
        )
        db.add(db_obj)
        db.commit()
        db.refresh(db_obj)
